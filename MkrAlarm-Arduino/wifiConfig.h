...
// The following include file just defines the macros MY_SSID and MY_PASSPHRASE, so it needs to be created first
// (i.e. it is not included with this source file distribution to keep those values secret)
...
char wpa_passphrase[] = MY_PASSPHRASE;
...
#define IS_IGNORE_WIFI101_SHIELD(p)  ((p) == 10 || (IS_PIN_SPI(p) && (p) != SS) || (p) == 7)
